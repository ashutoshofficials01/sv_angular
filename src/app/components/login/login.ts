...
export class Login implements OnInit {
  ...
  checkLogin(logEmailId: String, logPassword: String, logRole: String) {
    this.loginService
      .login(logEmailId, logPassword, logRole)
      .subscribe((outcome: loginCheckDto) => {
        console.log('outcome :: ', outcome);
        if (outcome.statusId == 1) {
          this.router.navigate(['/dashboard'], {
            queryParams: { role: outcome.role, id: outcome.userId },
          });
  ...
  validateEmail() {
    ...
    this.emailError = !emailRegex.test(this.logEmailId);
  }
  ...
  handleLogin() {
    ...
    if (!this.logEmailId || !this.logPassword || this.emailError) {
      return;
    }

    this.checkLogin(this.logEmailId, this.logPassword, 'personal');
  }
